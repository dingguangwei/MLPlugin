export default function (server){
    ...
    //Add a route to retrieve the health state of the ES cluster
    //You can find more in:
    //https://www.elastic.co/blog/kibana-plugin-developers-meet-elasticsearch-clusters
    // @author : dgw
    server.route({
        path: '/api/elasticsearch_status/cluster_health',
        method: 'GET',
        handler(req, reply){
            //call和callNum2均可
            call(req, 'cluster.health').then(response => {
                // console.log(`集群信息: #{response}`);
    ...
    server.route({
        path: '/api/elasticsearch_status/indices_analyze',
        method: 'GET',
        handler(req, reply){
            call(req, 'indices.analyze').then(response => {
                console.log(`分析索引: #{response}`);
                // reply(response.status);
                reply(response);
            })
        }
    ...
    server.route({
        path: '/api/elasticsearch_status/taste_event',
        method: 'GET',
        handler(req, reply){
            //call和callNum2均可
            call(req, 'taste.event').then(response => {
                console.log(`丁：taste.event: #{response}`);
                // reply(response.status);
                reply(response);
            })
        }
